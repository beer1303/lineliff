<template>
  <div>
    ...
    <v-container class="pt-0 pb-0">
      <v-row>
        <v-col cols="12">
          <div class="mt-4 text-primary text-title text-center">
            {{ list.date }}
          </div>
        </v-col>
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div
            v-if="index > 0"
            class="w-100 mt-3 text-center my-btn text-primary"
            @click="back"
          >
            Back
          </div>
  ...
  methods: {
    ...
    next() {
      if (this.index == this.workshops.length - 1) {
        ...
        this.$axios
          .patch(
            `https://lineliff-nuxt-default-rtdb.asia-southeast1.firebasedatabase.app/workshops/line:001.json`,
            {
